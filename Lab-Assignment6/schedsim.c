...
of processes. This array is initially a copy of *plist[].bt* (all 
processes burst times)
     2. Store waiting times of processes in plist[].wt. Initialize this 
array as 0.
     3. Initialize time : t = 0
     4. Keep traversing the all processes while all processes are not 
done. Do following for i'th process if it is not done yet.
        - If rem_bt[i] > quantum
          (i)  t = t + quantum
          (ii) bt_rem[i] -= quantum;
...
lap.
         ...
         - Calculate waiting time for each completed process. *wt[i]= 
Completion time - arrival_time-burst_time*
  ...
  while (finished != n) {
    ...
    if (curr_time >= plist[0].art) {
      ...
      if (left_time[curr_proc] == 0) {
        ...
        plist[curr_proc].wt = curr_time - plist[curr_proc].bt - 
plist[curr_proc].art;
      }
    } else {
      curr_time++;
