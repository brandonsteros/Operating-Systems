...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
   ...
   /*
   * 1. Insert newblk into list l in ascending order based on the START 
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   /*
   * 1. Insert newblk into list l in ascending order based on the 
blocksize.
   ...
   * 
   *         Insert newblk After Current Node if:   blocksize >= 
curr_blocksize
   * 
   *    USE the compareSize()
   */
...
void list_coalese_nodes(list_t *l){ 
  /*
   * 1. Assuming you have passed in a sorted list of blocks based on 
addresses in ascending order
   * 2. While list is not empty,
   *    a. compare two nodes at a time to see if the prev.END + 1 == 
current.START, if so, they are physically adjacent
...
bool list_is_in_by_pid(list_t *l, int pid){ 
  ...
  /* Iterate through the list to find a node with a blk that has blk->pid 
= pid
   * 
   * USE the comparePID()
   * 
