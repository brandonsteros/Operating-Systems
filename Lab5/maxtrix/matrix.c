...
int main() {
  ...
  // of matA and matB.
  void* computeProduct(void* args) { // pass in the number of the ith 
thread
    struct args_t* arg = (struct args_t*) args;
    int offset = arg->row * MAX;
    for (int i=0; i<MAX; i++) {
  ...
  for (int i=0; i<MAX; i++) {
    ...
    args[i].row = i;
    int error = pthread_create(&threads[i], NULL, &computeSum, (void *) 
&args[i]);
    if (error) {
  ...
  for (int i=0; i<MAX; i++) {
    ...
    args[i].row = i;
    int error = pthread_create(&threads[i], NULL, &computeDiff, (void *) 
&args[i]);
    if (error) {
      printf("Creation of threads failed!");
      exit(1);
  ...
  for (int i=0; i<MAX; i++) {
    ...
    args[i].row = i;
    int error = pthread_create(&threads[i], NULL, &computeProduct, (void 
*) &args[i]);
    if (error) {
      printf("Creation of threads failed!");
      exit(1);
