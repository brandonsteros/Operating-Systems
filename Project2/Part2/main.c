...
/**
 * Thread function that represents a cook in the restaurant. A cook 
should:
...
int main() {
  ...
  for (int i=0; i<NUM_COOKS; i++) {
    cook_id[i] = i+1;
    pthread_create(&cook[i], NULL, &BENSCHILLIBOWLCook, (void*) 
&(cook_id[i]));
  }
  
  for (int i=0; i<NUM_CUSTOMERS; i++) {
    cust_id[i] = i+1;
    pthread_create(&customer[i], NULL, &BENSCHILLIBOWLCustomer, (void*) 
&(cust_id[i]));
  }
